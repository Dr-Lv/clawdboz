...
1. 前往 [飞书开放平台](https://open.feishu.cn)（`open.feishu.cn`）登录开发者账号
2. 点击「开发者后台」→「创建企业自建应用」
3. 填写应用名称和描述，点击「创建」
...
> 📋 **快捷参考**：GitHub 仓库中的 [`feishu_permissions.json`](./feishu_permissions.json) 包含完整的权限列表，可直接用于批量配置。
