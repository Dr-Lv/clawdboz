...
class Session:
    ...
    def __init__(self, session_id: str, bot_id: str, session_type: str = "single",
                 base_workplace: str = None, is_group: bool = False, platform: str = "webchat"):
        """
        初始化会话
        ...
        Args:
            ...
            is_group: 是否是群聊
            platform: 平台类型 feishu/webchat
        """
        self.session_id = session_id
        self.bot_id = bot_id
        self.session_type = session_type
        self.is_group = is_group
        self.platform = platform  # feishu 或 webchat
    ...
    def _get_bot_work_dir(self) -> str:
        """获取 Bot 工作目录路径（用于加载 skills）"""
        return os.path.join(self.base_workplace, f"workplace_{self.bot_id}")
    ...
    def _setup_workspace(self):
        ...
        dirs = [
            ...
            os.path.join(self.work_dir, "user_files"),  # 统一存放用户上传的文件和图片
            os.path.join(self.work_dir, ".agents", "skills"),
    ...
    def _create_session_config(self):
        ...
        config = {
            ...
            "platform": self.platform,  # 标识会话平台类型
            "work_dir": self.work_dir,
            "paths": {
                "workplace": self.work_dir,
                "user_files": os.path.join(self.work_dir, "user_files"),  # 统一存放文件和图片
                "skills_dir": os.path.join(self.work_dir, ".agents", "skills"),
                "memory_dir": os.path.join(self.work_dir, "memory"),
                "mcp_config": os.path.join(self.work_dir, ".agents", "mcp.json"),
            },
            "logs": {
    ...
    def init_acp_client(self, system_prompt: str = None):
        ...
        # 获取 Bot 工作目录（用于加载 skills）
        bot_work_dir = self._get_bot_work_dir()
        
        # 创建会话级 ACP 客户端
        self.acp_client = ACPClient(
            bot_ref=None,  # 不使用 bot 引用，直接管理
            session_work_dir=self.work_dir,
            bot_work_dir=bot_work_dir,  # 传入 bot 工作目录，用于加载 skills
...
class SessionManager:
    ...
    def create_session(self, session_id: str, bot_id: str, 
                       ...
                       is_group: bool = False,
                       member_bot_ids: List[str] = None,
                       platform: str = "webchat") -> Session:
        """
        创建新会话
