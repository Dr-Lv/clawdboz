...
class BotManager:
    ...
    def register(self, bot_id: str, app_id: str, app_secret: str, 
        ...
        # 为每个 Bot 创建独立工作目录（使用 workplace_{bot_id} 格式）
        bot_work_dir = os.path.join(self.base_workplace, f"workplace_{bot_id}")
        ...
        os.makedirs(os.path.join(bot_work_dir, ".agents", "skills"), exist_ok=True)
        ...
        config = {
            ...
            "paths": {
                ...
                "mcp_config": os.path.join(bot_work_dir, ".agents", "mcp.json"),
        ...
        try:
            ...
            # 重新加载 ACP Client ID（因为 _work_dir 刚设置）
            if hasattr(bot, '_load_acp_client_id_from_bot_md'):
                bot._load_acp_client_id_from_bot_md()
            bot.name = bot_id  # 默认 name 为 bot_id
            if system_prompt:
                bot._system_prompt = system_prompt
                # 尝试从 system_prompt 提取 Bot 名称
                # 匹配 "你是XXX" 或 "名字叫 XXX"
                import re
                patterns = [
                    r'你是\*\*(.+?)\*\*[，,，]',
                ...
                for pattern in patterns:
                    ...
                    if match:
                        bot.name = match.group(1).strip()
                        break
            
            # 初始化默认定时任务（此时 _bot_id 已设置）
            if hasattr(bot, '_init_default_scheduler_tasks'):
