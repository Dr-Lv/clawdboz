...
def _ensure_project_files(work_dir: str, verbose: bool = False):
    """
    确保项目文件存在（.bot.md 和 bot_manager.sh）
    内部辅助函数，避免循环导入
    """
    try:
        # 尝试从 cli 导入（使用绝对导入避免循环）
        ...
        cli_py_path = current_file.parent / 'cli.py'

        if cli_py_path.exists():
            # 动态加载 cli.py
            spec = importlib.util.spec_from_file_location("clawdboz_cli", cli_py_path)
            cli_module = importlib.util.module_from_spec(spec)
            sys.modules["clawdboz_cli"] = cli_module
            spec.loader.exec_module(cli_module)
            return cli_module.ensure_bot_files(work_dir, verbose=verbose)
        else:
            raise ImportError(f"cli.py not found at {cli_py_path}")
    except Exception as e:
        # 如果导入失败，直接创建文件
        result = {'created': [], 'existing': [], 'errors': []}
        
        # 创建 .bot.md
        bots_md_path = os.path.join(work_dir, '.bot.md')
...
"""
            try:
                ...
                result['created'].append('.bot.md')
                if verbose:
                    print(f"[Bot] 创建 Bot 规则文件: .bot.md")
            except Exception as e:
                result['errors'].append(f'.bot.md: {e}')
        else:
            result['existing'].append('.bot.md')
...
class Bot:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # 设置环境变量，让 memory 模块知道 bot 的工作目录
        os.environ['CLAWDBOZ_BOT_WORK_DIR'] = self.work_dir
        
        # 自动创建 .bot.md 和 bot_manager.sh（如果不存在）
        ...
        # 设置默认 bot_id（如果没有指定）
        if not hasattr(self, '_bot_id'):
            self._bot_id = kwargs.get('bot_id', 'default')
        ...
        # 传递 _bot_id 到内部的 LarkBot
        self._bot._bot_id = self._bot_id

        # 从 .bot.md 加载 ACP Client ID
        self._load_acp_client_id_from_bot_md()
    ...
    def _load_configuration(
        ...
        **kwargs
        ...
        # 6. 从 bots.{bot_id} 读取飞书配置（新的多 Bot 配置结构）
        # 优先使用 bots 下的配置，兼容旧版的顶层 feishu 配置
        # 从 kwargs 获取 bot_id，或从工作目录名提取（如 workplace_55 -> 55）
        bot_id = kwargs.get('bot_id', None)
        if not bot_id:
            # 从工作目录名提取 bot_id
            work_dir_name = os.path.basename(self.work_dir)
            if work_dir_name.startswith('workplace_'):
                bot_id = work_dir_name[len('workplace_'):]
            else:
                bot_id = 'default'
        bots_config = config.get('bots', {})
        if bot_id in bots_config:
            bot_feishu_config = bots_config[bot_id].get('feishu', {})
            if bot_feishu_config:
                config['feishu'] = bot_feishu_config
                print(f"[Config] 从 bots.{bot_id}.feishu 加载配置")

        # 7. 从 .bot.md 读取 Verification Token 和 Encrypt Key（如果 config 中没有）
        feishu_config = config.get('feishu', {})
        if not feishu_config.get('verification_token') or not feishu_config.get('encrypt_key'):
            bot_md_path = os.path.join(self.work_dir, '.bot.md')
            if os.path.exists(bot_md_path):
                try:
                    with open(bot_md_path, 'r', encoding='utf-8') as f:
                        for line in f:
                            if line.startswith('Verification Token:'):
                                verification_token = line.replace('Verification Token:', '').strip()
                                if verification_token and not feishu_config.get('verification_token'):
                                    if 'feishu' not in config:
                                        config['feishu'] = {}
                                    config['feishu']['verification_token'] = verification_token
                                    print(f"[Config] 从 .bot.md 加载 Verification Token")
                            elif line.startswith('Encrypt Key:'):
                                encrypt_key = line.replace('Encrypt Key:', '').strip()
                                if encrypt_key and not feishu_config.get('encrypt_key'):
                                    if 'feishu' not in config:
                                        config['feishu'] = {}
                                    config['feishu']['encrypt_key'] = encrypt_key
                                    print(f"[Config] 从 .bot.md 加载 Encrypt Key")
                except Exception as e:
                    print(f"[Config] 从 .bot.md 读取配置失败: {e}")
        
        # 8. 没有配置文件时，自动创建 config.json
        if not config_file_exists and config_updated:
            try:
                bot_id = getattr(self, '_bot_id', 'default')
                # 构建完整的默认配置（使用新的 bots.{bot_id} 结构）
                default_config = {
    ...
    def _load_acp_client_id_from_bot_md(self):
        """从 .bot.md 加载 ACP Client ID"""
        try:
            bot_md_path = os.path.join(self.work_dir, '.bot.md')
            if os.path.exists(bot_md_path):
                with open(bot_md_path, 'r', encoding='utf-8') as f:
                    for line in f:
                        if line.startswith('ACP Client ID:'):
                            acp_client_id = line.replace('ACP Client ID:', '').strip()
                            if acp_client_id:
                                self._acp_client_id = acp_client_id
                                self._bot._acp_client_id = acp_client_id
                                print(f"[Bot] 从 .bot.md 加载 ACP Client ID: {acp_client_id}")
                            break
        except Exception as e:
            print(f"[Bot] 从 .bot.md 加载 ACP Client ID 失败: {e}")
    ...
    def _init_default_scheduler_tasks(self):
        ...
        委托给内部的 LarkBot 实例
        """
        if hasattr(self._bot, '_init_default_scheduler_tasks'):
            # 将外部的 _bot_id 传递给内部的 LarkBot
            if hasattr(self, '_bot_id'):
                self._bot._bot_id = self._bot_id
            self._bot._init_default_scheduler_tasks()
