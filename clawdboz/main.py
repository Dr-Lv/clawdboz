...
class MonitoredWSClient(WSClient):
    ...
    async def _connect(self) -> None:
        ...
        try:
            ...
            # 从 config.json 读取并覆盖心跳间隔
            hb = CONFIG.get('scheduler', {}).get('heart_beat')
            if hb is not None:
                self._ping_interval = int(hb)
                logger.info(f"[CONNECT] WebSocket 连接成功 - 心跳间隔设置为 {self._ping_interval} 秒")
            else:
...
def main():
    ...
    # 创建事件处理器 - 优先从 bot_instance 获取，否则从 CONFIG 获取
    verification_token = getattr(bot_instance, 'verification_token', '') or CONFIG.get('feishu', {}).get('verification_token', '')
    encrypt_key = getattr(bot_instance, 'encrypt_key', '') or CONFIG.get('feishu', {}).get('encrypt_key', '')
